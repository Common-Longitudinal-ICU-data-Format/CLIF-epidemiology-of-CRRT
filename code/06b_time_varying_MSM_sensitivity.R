####### MSM Sensitivity Analysis: 24h/48h Intervals (0-24h, 24-48h) ############
#
# Copied from 06_time_varying_MSM.R (primary 12h analysis).
# Key changes:
#   - Filters to eligible_48h_sensitivity == 1
...
# Filter to 48h sensitivity cohort
stopifnot("eligible_48h_sensitivity" %in% names(df))
df <- df %>% dplyr::filter(eligible_48h_sensitivity == 1)
cat("Filtered to 48h sensitivity cohort:", nrow(df), "rows\n")
...
required_vars <- c(
  ...
  "crrt_dose_0_24", "crrt_dose_24_48",
  # Labs at t=0 and t=24
  "lactate_0", "bicarbonate_0", "potassium_0",
  "lactate_24", "bicarbonate_24", "potassium_24",
...
dose_cols <- c("crrt_dose_ml_kg_hr_0", "crrt_dose_0_24", "crrt_dose_24_48")                                                                                           
...
sample_chars <- data.frame(
  ...
  # CRRT dose mean 0-24h
  crrt_dose_0_24_mean = mean(df_complete$crrt_dose_0_24, na.rm = TRUE),
  crrt_dose_0_24_sd = sd(df_complete$crrt_dose_0_24, na.rm = TRUE),
  crrt_dose_0_24_median = median(df_complete$crrt_dose_0_24, na.rm = TRUE),
  crrt_dose_0_24_q25 = quantile(df_complete$crrt_dose_0_24, 0.25, na.rm = TRUE),
  crrt_dose_0_24_q75 = quantile(df_complete$crrt_dose_0_24, 0.75, na.rm = TRUE),
  crrt_dose_0_24_min = min(df_complete$crrt_dose_0_24, na.rm = TRUE),
  crrt_dose_0_24_max = max(df_complete$crrt_dose_0_24, na.rm = TRUE),

  # CRRT dose mean 24-48h
  crrt_dose_24_48_mean = mean(df_complete$crrt_dose_24_48, na.rm = TRUE),
  crrt_dose_24_48_sd = sd(df_complete$crrt_dose_24_48, na.rm = TRUE),
  crrt_dose_24_48_median = median(df_complete$crrt_dose_24_48, na.rm = TRUE),
  crrt_dose_24_48_q25 = quantile(df_complete$crrt_dose_24_48, 0.25, na.rm = TRUE),
  crrt_dose_24_48_q75 = quantile(df_complete$crrt_dose_24_48, 0.75, na.rm = TRUE),
...
# 2. Mean dose 0-24h
crrt_dose_hist_0_24 <- plot_dose_hist(
  df_complete, "crrt_dose_0_24", "Mean CRRT Dose (0-24h)")
crrt_dose_hist_0_24

ggsave(file.path(output_dir, paste0(SITE_NAME, "_", dose_label,
                                    "_hist_crrt_dose_0_24.png")),
       crrt_dose_hist_0_24, width = 6, height = 4)

# 3. Mean dose 24-48h
crrt_dose_hist_24_48 <- plot_dose_hist(
  df_complete, "crrt_dose_24_48", "Mean CRRT Dose (24-48h)")
crrt_dose_hist_24_48

ggsave(file.path(output_dir, paste0(SITE_NAME, "_", dose_label,
                                    "_hist_crrt_dose_24_48.png")),
       crrt_dose_hist_24_48, width = 6, height = 4)
...
dose_long <- df_complete %>%                                                                                                                                          
  select(encounter_block, crrt_dose_ml_kg_hr_0, crrt_dose_0_24, crrt_dose_24_48) %>%
  pivot_longer(
    cols = c(crrt_dose_ml_kg_hr_0, crrt_dose_0_24, crrt_dose_24_48),
    names_to = "window",
    values_to = "dose"
  ) %>%
  mutate(
    window = factor(window,
                    levels = c("crrt_dose_ml_kg_hr_0", "crrt_dose_0_24", "crrt_dose_24_48"),
                    labels = c("At Initiation", "Mean 0-24h", "Mean 24-48h")
    )
  )
...
crrt_dose_hist_combined <- ggplot(dose_long, aes(x = dose, fill = window)) +
  ...
  labs(
    title = paste0("CRRT Dose Distribution Over First 48h (Sensitivity)",
                   " (Cutoff = ", dose_cutoff, " mL/kg/hr)"),
    x = "CRRT Dose (mL/kg/hr)",
    y = "Count",
    fill = "Time Window"
...
# Add A_0 and A_24 labels (sensitivity: 24h intervals)
df_msm_wide <- df_complete %>%
  mutate(
    A_0  = as.integer(crrt_dose_0_24  >= dose_cutoff),
      # treatment for interval 0-24h
    A_24 = as.integer(crrt_dose_24_48 >= dose_cutoff)
      # treatment for interval 24-48h
  )
...
df_msm_wide <- df_msm_wide %>%
  mutate(
    A_0_f  = factor(A_0,  levels=c(0,1),
                    labels=c(paste0("<",dose_cutoff), paste0(">=",dose_cutoff))),
    A_24_f = factor(A_24, levels=c(0,1),
                    labels=c(paste0("<",dose_cutoff), paste0(">=",dose_cutoff)))
  )
...
# Final weight for outcome model = weight after t=24 decision (covers 0-24 and 24-48)
df_w_final <- df_w %>%
  filter(tpt == 24) %>%
...
### ---- I. Weight summaries (final MSM weights at t=24) ----
...
plot_overlap_t24 <- plot_overlap_timepoint(
  df_w, 24, output_dir, SITE_NAME, dose_label, dose_cutoff)
...
bal_t24 <- run_balance_timepoint(
  df_w, tt = 24, output_dir, SITE_NAME, dose_label,
  dose_cutoff, pretty_names_msm, baseline_terms, tv_terms
)
...
# Ensure df_out_msm contains: w_msm, high_count, A_0, A_24
...
df_msm_tableS1 <- df_out_msm %>%
  mutate(
    ...
    crrt_group = factor(
      case_when(
        A_0 == 0 & A_24 == 0 ~ "low_low",
        A_0 == 0 & A_24 == 1 ~ "low_high",
        A_0 == 1 & A_24 == 0 ~ "high_low",
        A_0 == 1 & A_24 == 1 ~ "high_high"
      ),
...
plot_cif_death <- ggplot(cif_df, aes(x = time, y = cif_death, color = crrt_strategy,
  ...
  labs(
    ...
    color = "CRRT Strategy (0-48h)",
    fill = "CRRT Strategy (0-48h)"
  )
